...
var databasestore;
var allConnections;
var hostAddr = "http://localhost:3000/";
...
function saveDatabaseConnection() {
	$("#save_spinner").show();
	$.ajax({
	    url : hostAddr+"data/service/saveconnection",
		type : "POST",
...
function testDatabaseConnection() {
	$("#test_spinner").show();
	$.ajax({
	    url : hostAddr+"data/service/testconnection",
		type : "GET",
		data : $("#connectionDataForm").serialize(),
		statusCode : {
			200 : function() {
...
function deleteSelectedConnection() {
	...
	apprise('Are you sure you want to delete connection <strong>' + selected + '</strong>, the connection might be used by your reports?', {
		...
		if (r) {
			$.ajax({
			    url : hostAddr+"data/service/deleteconnection",
				type : "GET",
				data : {
					connection : selected
				},
...
function selectCSVdb(csvfileid) {
	...
	if (!data) {
		$.ajax({
		    url : hostAddr+"data/service/filedatabase",
			type : "GET",
			data : {
				fileid : csvfileid
			}
...
function initializeConnectionsDialogComponents() {
	...
	$.ajax({
	    url : hostAddr+"data/service/alldatabases",
		type : "GET"
	}).done(function(data) {
...
function deleteUploadedFile() {
	...
	if (csvdataid) {
		apprise('Are you sure you want to delete ' + $("#csvdata option:selected").text() + ' file database? <strong> ', {
			...
			if (r) {
				$.ajax({
				    url : hostAddr+"data/service/deletefiledatabase",
					type : "GET",
					data : {
						fileid : csvdataid
					},
